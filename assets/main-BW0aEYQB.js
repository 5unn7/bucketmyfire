...
${ie}`,f=new URLSearchParams(location.search),pe=["m","autostart","qa","ffa","province","daily","editor","dev","heliview","kit","tune"],le=pe.some(e=>f.has(e));le?queueMicrotask(fe):me();let g=!1;function fe(){if(g)return;g=!0,document.body.classList.add("bmf-playing");const e=document.getElementById("game");e&&(e.innerHTML=""),document.getElementById("fd-boot")?.remove(),!f.has("qa")&&!f.has("autostart")&&ce(),x(()=>import("./main-Ry7GsRDt.js").then(t=>t.a8),__vite__mapDeps([0,1]),import.meta.url)}function ce(){if(document.getElementById("bmf-splash"))return;if(!document.getElementById("bmf-splash-css")){const i=document.createElement("style");i.id="bmf-splash-css",i.textContent=se,document.head.appendChild(i)}const e=document.createElement("div");for(const[i,p]of Object.entries(re))e.setAttribute(i,p);e.innerHTML=ne,document.body.appendChild(e);const t=1100,o=performance.now();let a=!1,n=0;const r=()=>{a||(a=!0,n&&clearTimeout(n),e.classList.add("bmf-hide"),setTimeout(()=>e.remove(),550))},s=()=>{if(a||n)return;const i=t-(performance.now()-o);i<=0?r():n=window.setTimeout(r,i)};window.addEventListener("bmf:ready",s),setTimeout(r,12e3)}function me(){L(),T(),C(),A(),I(),Ee();const e=document.getElementById("game");if(!e)return;const t=document.createElement("div");t.className="bmf-app front",t.innerHTML=P+xe(),e.innerHTML="",e.appendChild(t),document.getElementById("fd-boot")?.remove(),F(t,13),z(t),de(t),v(-1),ge(),ue(t),te(t),ye(),ve(t),f.has("map")&&y()}function de(e){const t=e.querySelector("#fhome-pilot");if(!t)return;const o=B();if(!o?.name){t.classList.add("is-new"),t.innerHTML='<span class="fpb-cs">New pilot</span><span class="fpb-meta"><span class="fpb-hint">Fly to earn your rank</span></span>';return}const a=O(R());t.innerHTML=`<span class="fpb-cs"></span><span class="fpb-meta"><span class="rank" style="--rk:${a.color}"><i></i>${a.name}</span><span class="fpb-pts pts-ic mono">${l("spark")}<b>${M().toLocaleString("en-US")}</b> pts</span></span>`;const n=t.querySelector(".fpb-cs");n&&(n.textContent=o.name.toUpperCase())}function v(e){const t=document.getElementById("fhome-pilots-live");if(!t)return;const o=t.querySelector("b"),a=t.querySelector("span");e>=1?(o&&(o.textContent=e.toLocaleString("en-US")),a&&(a.textContent=e===1?"Pilot live":"Pilots live")):(o&&(o.textContent=""),a&&(a.textContent="Open Skies live"))}const he=6e3,be=4e3;async function ge(){let e;try{e=await x(()=>import("./openSkies-BkVayIv8.js"),[],import.meta.url)}catch{return}if(!e.openSkiesConfigured())return;const t=e.connectOpenSkies(_(new Date),{id:"fd-listener",name:"",heli:""},he);if(!t)return;const o=()=>v(t.remotes().length),a=window.setTimeout(o,1400),n=window.setInterval(o,be);window.addEventListener("pagehide",()=>{window.clearTimeout(a),window.clearInterval(n),t.close()},{once:!0})}function xe(){return`
${H()}
