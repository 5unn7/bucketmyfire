...
function heliCrane(): string {
  let s = heliPad();
  s += shadow(1.6, 1.55, 1.05);
  // tall stilt-legs of the skycrane
  s += box(0.95, 1.0, 0.5, 0.16, 0.16, 0.5, '#3a3e44');
  s += box(0.95, 1.9, 0.5, 0.16, 0.16, 0.5, '#3a3e44');
  s += box(2.2, 1.0, 0.5, 0.16, 0.16, 0.5, '#3a3e44');
  s += box(2.2, 1.9, 0.5, 0.16, 0.16, 0.5, '#3a3e44');
  // slung water tank under the spine
  s += box(1.2, 1.2, 0.62, 1.3, 0.7, 0.34, '#9aa0a6', { top: '#c2c7cc' });
  // long heavy spine/body (yellow) + boom + tail fin
  s += box(0.25, 1.4, 1.02, 1.0, 0.22, 0.22, '#caa233');
  s += box(0.1, 1.32, 1.05, 0.2, 0.36, 0.6, '#d8a12a');
  s += box(0.85, 1.0, 0.98, 1.75, 1.0, 0.6, '#d8a12a', { top: '#ecc760' });
  s += poly([P(2.6, 1.1, 1.4), P(2.6, 1.9, 1.4), P(2.6, 1.9, 1.02), P(2.6, 1.1, 1.02)], '#27333d');
  // mast + wide heavy rotor
  s += box(1.66, 1.42, 1.58, 0.22, 0.22, 0.22, '#3a3e44');
  s += box(-0.05, 1.49, 1.78, 3.5, 0.13, 0.07, '#2b2f35');
  s += box(1.7, -0.2, 1.78, 0.13, 3.5, 0.07, '#2b2f35');
  s += box(1.58, 1.4, 1.77, 0.24, 0.24, 0.08, '#50555c');
  return s;
}

function heliScout(): string {
  let s = heliPad();
  s += shadow(1.55, 1.5, 0.85);
  // skids
  s += box(1.0, 1.15, 0.55, 1.3, 0.14, 0.09, '#33373d');
  s += box(1.0, 1.78, 0.55, 1.3, 0.14, 0.09, '#33373d');
  // long thin tail boom + small upright fin
  s += box(0.25, 1.45, 0.92, 1.1, 0.14, 0.14, '#cdd2d6');
  s += box(0.18, 1.4, 0.95, 0.14, 0.24, 0.42, '#3d8fb0');
  // little bubble cabin (teal, bright glass top)
  s += box(1.3, 1.1, 0.64, 0.95, 0.85, 0.62, '#3d8fb0', { top: '#bfe7f2' });
  s += poly([P(2.25, 1.16, 1.16), P(2.25, 1.79, 1.16), P(2.25, 1.79, 0.78), P(2.25, 1.16, 0.78)], '#214b58');
  // mast + big rotor (nimble scout has an oversized disc)
  s += box(1.66, 1.4, 1.26, 0.16, 0.16, 0.2, '#3a3e44');
  s += box(0.05, 1.45, 1.46, 3.2, 0.11, 0.06, '#2b2f35');
  s += box(1.66, -0.1, 1.46, 0.11, 3.2, 0.06, '#2b2f35');
  s += box(1.58, 1.34, 1.45, 0.18, 0.18, 0.07, '#50555c');
  return s;
}
